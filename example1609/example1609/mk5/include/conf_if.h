...
typedef enum
{
  /// enforce compile-time limits
  CONF_MODE_ENFORCING,
  /// limit to compile-time limits
  CONF_MODE_LIMIT,
  /// ignore compile-time limits. HERE BE DRAGONS!
  CONF_MODE_ALLOW
} tConfReadMode;
...
//------------------------------------------------------------------------------
  ...
  void Conf2_Init(const char * pTag);
  void Conf2_Exit(const char * pTag);
  ...
  int Conf2_DebugUpdate(void);
   ...
   // disable generation of QSMSG_SYS_CONF broadcast
   void Conf2_Noisy(void);
   // enable generation of QSMSG_SYS_CONF broadcast
   void Conf2_Quiet(void);
