...
/// Protocol version 3
#define ETSIFAC_PROTOCOLVERSION3 3
...
typedef enum ETSIFACGNProfile
{
  ...
  /// communication profile: long range (Cellular-Uu)
...
typedef enum ETSIFACGNTC
{
  ...
  /// Channel offload setting, set or not set [for dual G5 radios]
  ETSIFAC_GN_TC_OFFLOAD  = 0x40,

  /// Traffic class DP0 to DP32
  /// Traffic class DP0 has highest priority class and no transmit power restrictions inside Protected Communication Zones
  ETSIFAC_GN_TC_ITS_DP0  = 0x00,
  /// Traffic classes DP1 to DP8 used on SCH0 (CCH), will be mapped to approprate priority class (Access Class in G5 or PPPP in PC5)
  ...
  /// Traffic classes DP9 to DP32 not used on SCH0 (CCH), will be sent on SCHX (SCH) [for dual G5 radios]
...
/// Struct for FAC Init parameters that can override the configuration file settings
typedef struct ETSIFacInitData
{
  /// Initial Station ID, 0 means don't use this value (i.e. rely on configuration setting)
  uint32_t InitStationId;
  /// Initial Station type (ITSStationType) ITSStationType_unknown means don't use this value (i.e. rely on configuration setting)
  uint8_t InitStationType;
  /// Initial Station role (ITSVehicleRole) ITSVehicleRole_Default means don't use this value (i.e. rely on configuration setting)
  uint8_t InitStationRole;
