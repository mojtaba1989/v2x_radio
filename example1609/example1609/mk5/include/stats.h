...
typedef enum Stats_StackCryptoResult
{
  ...
  /// No root trust lists (e.g. ECTL)
  STACK_CRYPTO_NO_ROOT_TRUST_LIST,
  /// id change is ongoing
  STACK_CRYPTO_ID_CHANGE_ONGOING,
  /// Not initialised
  STACK_CRYPTO_NOT_INITIALIZED,
  /// Signing hardware failure (e.g. HSM problem)
...
typedef struct Stats_StackStats
{
  ...
  tStats_StackCryptoResult LastSysCryptoResult;
...
/// Update the System crypto status (e.g. at initialisation of security system)
void Stats_Report_SysCrypto(tStats_StackCryptoResult CryptoResult);
