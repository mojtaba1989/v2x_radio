...
typedef enum TAThreatType
{
  ...
  // V2I (SPAT/MAP)
  TA_THREATTYPE_TLI      = (UINT32_C(1)<<18), ///< Green Light Speed Advice
...
typedef struct TAAlert
{
  ...
  union
  {
    ...
    struct
    {
      ...
      // indicate where the RV is
      // must be either TC_CLASSIF_IS_LEFT or TC_CLASSIF_IS_RIGHT
      tTCClassif RVPosition;
    } BSW_LCW;
    /// Curve Speed Warning
    ...
    struct
    {
      ...
      int8_t IsMobile;
      /// Is the RWW winter service. -1 if unknown, 0 = not winter, 1 = winter
      int8_t IsWinter;
    ...
    /// Type == TA_THREATTYPE_TLI
    struct
    {
      /// Current type of TLI @ref eTATliType
      uint8_t AlertType;
    ...
    struct
    {
      /// GB/T 29100
      int EventType;
      /// On the path or inside the radius. See RSICNPointInRegionResult
      uint8_t Region;
    } HLW_CN;
    /// Traffic Jam Warning
 ...
 * @param[in] pCurrentAlert Current TA alert state, HMI is displaying
 * @param[in,out] pPriv Provided to TA_RegisterApplication()
 * @param[in,out] pScratchpad Per-remote scratchpad for the registered app
 *
 ...
 * cancelled).
 *
 * To report an alert, set up Type/Level/TimeToEvent_s/DistanceToEvent_m and 
 * alert-specific fields in pAlert. Other fields in pAlert are set up by TA 
 * automatically.
 */
...
/**
 * @brief Assess a non-RV Threat (RWW, TLI etc.)
 *
 * @param[in] pHV Data for the HV
 * @param[out] pAlert RV alert state (modifed by this func)
 * @param[in] pCurrentAlert Current TA alert state, HMI is displaying
 * @param[in,out] pPriv Provided to TA_RegisterApplication()
 *
 * This function is provided by the Application to evaluate a given threat that
 * is independant of Remote Vehicles. See @ref fTA_ThreatApplication.
 */
