 ...
 *     {
 *       // Check PDU header
 *       if (pData->pPDU->messageID == ITSMessageId_ID_cam)
 *       {
...
/// @return CAM Status Code
/// This allows applications to attempt to modulate the CAM rate.
/// The provided value is bounded:
/// ItsCamInitInterval <= Interval <= ItsCamSlowInterval
...
/// @return CAM Status Code
/// This in intended to allow DCC (Distributed Congestion Control) to
/// modulate the CAM rate.
/// The provided value is bounded:
/// ItsCamInitInterval <= Interval <= ItsCamSlowInterval
tCAMStatusCode CAM_AdjustDccInterval(uint16_t Interval);

/// @brief Acquire current CAM stationID and role information
/// @return CAM information
/// This returns the current station ID along with the current role
