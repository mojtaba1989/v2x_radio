...
// #16719: RSU1.0 Fix core dumps
#if defined(__QNX__) || 1
#define CONF_DBG(fmt, ...)
#define TID ((int)-1)
#else
...
#define CONF_DBG_ALLOC_TAG(pPtr, Typ, pTag)             \
  do                                                    \
  {                                                     \
    CONF_DBG("CONF2:%s:%s tid=%d alloc(%s): %p[%zu]\n", \
             pTag, __FUNCTION__,                        \
...
#define CONF_DBG_ALLOC(pPtr, Typ)               \
  CONF_DBG_ALLOC_TAG(pPtr, Typ, "")

#define CONF_DBG_SIZE_TAG(pPtr, Size, pTag)             \
  do                                                    \
  {                                                     \
    CONF_DBG("CONF2:%s:%s tid=%d alloc(): %p[%zu]\n",   \
             pTag, __FUNCTION__,                        \
             (int)syscall(SYS_gettid),                  \
             (const void *)pPtr,                        \
             Size);                                     \
...
#define CONF_DBG_SIZE(pPtr, Size)               \
  CONF_DBG_SIZE_TAG(pPtr, Size,"")

#define CONF_DBG_FREE_TAG(pPtr, pTag)                   \
  do                                                    \
  {                                                     \
    CONF_DBG("CONF2:%s:%s tid=%d free(%s): %p\n",       \
             pTag, __FUNCTION__,                        \
             (int)syscall(SYS_gettid),                  \
             #pPtr,                                     \
             (const void *)pPtr);                       \
...
#define CONF_DBG_FREE(pPtr)                     \
  CONF_DBG_FREE_TAG(pPtr,"")
...
void Conf_Stash(const char * pFilename, const bool OK);
