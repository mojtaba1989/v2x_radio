 ...
 *     {
 *       // Check PDU header
 *       if (pData->pPDU->messageID == ITSMessageId_ID_denm)
 *       {
...
#define DENM_GetRelevanceDistanceFromMetre(Metres) \
  ((Metres) <    50) ? ITSStandardLength3b_lessThan50m : \
  ((Metres) <   100) ? ITSStandardLength3b_lessThan100m : \
  ((Metres) <   200) ? ITSStandardLength3b_lessThan200m : \
  ((Metres) <   500) ? ITSStandardLength3b_lessThan500m : \
  ((Metres) <  1000) ? ITSStandardLength3b_lessThan1000m : \
  ((Metres) <  5000) ? ITSStandardLength3b_lessThan5km : \
  ((Metres) < 10000) ? ITSStandardLength3b_lessThan10km : \
...
typedef struct DENMMgmt
{
  ...
  /// validityDuration
  ITSDeltaTimeSecond repetitionDuration;
...
tDENMStatusCode DENM_SendDENMTrigger(const tDENMMgmt *pMgmt, const ITSDenmPayload *pDenm, tDENMActionID *pActionId);
...
/// @return DENM Status Code
tDENMStatusCode DENM_SendDENMUpdate(const tDENMMgmt *pMgmt, const ITSDenmPayload *pDenm, tDENMActionID ActionId);
...
/// @param ActionId ActionID provided by DENM_SendDENMTrigger (or user-defined action ID)
/// @return DENM Status Code
tDENMStatusCode DENM_SendDENMTermination(const tDENMMgmt *pMgmt, const ITSDenmPayload *pDenm, tDENMActionID ActionId);
...
/// @return DENM Status Code
tDENMStatusCode DENM_SendDENMTriggerWithId(const tDENMMgmt *pMgmt, const ITSDenmPayload *pDenm, tDENMActionID ActionId);
