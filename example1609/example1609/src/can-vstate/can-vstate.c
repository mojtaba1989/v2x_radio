...
#define D_LEVEL Example1609_Example1609_CANVState_DebugLevel

#define CANVSTATE_CANID 0x123
//For Extended CAN ID ((12 to 29 bits),  add/OR CAN_EFF_FLAG (=0x80000000)
//For example, if CAN ID being sent is 0x18fef100, then set as below:
//#define CANVSTATE_CANID 0x18fef100 + CAN_EFF_FLAG
...
static int CANVState_LoadConfig (tCANVState1609 *pCV,
                    ...
                    char *pConfigFileName)
{
  ...
  pCV = (tCANVState1609 *) malloc(sizeof(tCANVState1609));
  if (pCV == NULL)
  {
    Res = -ENOMEM;
    goto Error;
  }
  memset(pCV, 0, sizeof(tCANVState1609));
...
void CANVState_Close (tCANVState1609 *pCV)
{
  ...
  ShouldJoin = (pCV->ThreadState & CANVSTATE_THREAD_STATE_INIT) == CANVSTATE_THREAD_STATE_INIT;
  pthread_mutex_unlock(&pCV->Lock);
  if (ShouldJoin)
  {
